...
class ybhGreenSpider(BaseSpider):
    name = 'whappy'
    allowed_domains = ['whappy.ca']
    start_urls = ['https://shop.whappy.ca/']
    p_id = '990048'
    shop_name = 'Whappy Cannabis'
    ...
    def parse(self, response):
        ...
        yield {
			...
			"Active": "Yes",
			"Main image": response.xpath('//img[@alt="Whappy Cannabis"]/@src').get(),
			"Image 2": "",
			"Image 3": "",
			"Image 4": '',
			"Image 5": '', 
			...
			"Contact Name": "",
			"EmailPrivate": "",
			"Email": 'info@whappy.ca',
			"Social": "",
			"FullAddress": '2075 Lawrence Ave E, Scarborough, ON M1R 2Z4, Canada',
			"Address": '2075 Lawrence Ave E',
			"Additional Info": "",
			"Created": "",
			"Comment": "",
			"Updated": ""
        ...
        for category in categories:
            ...
            if len(url.split('/')) == 5 and url != 'https://shop.whappy.ca/':
    ...
    def parse_product(self, response):
        ...
        stock_count = response.xpath('//tr[@class="availability"]/td[2]/text()').get().strip().strip('(').strip(')')
        ...
        desc = remove_non_ascii(desc) if desc else ''
        try:
            thc = response.xpath('//div[@class="tabsPages"]').xpath('.//tr/td[2]/text()')[1].get()
            cbd = response.xpath('//div[@class="tabsPages"]').xpath('.//tr/td[2]/text()')[2].get()
        except:
            cbd = thc = ''
        terpenes = ''
        for item in response.xpath('//div[@class="page info active"]/p'):
            if item.xpath('text()').get() and 'Terpenes:' in item.xpath('text()').get():
                terpenes = item.xpath('text()').get().strip('Terpenes:').strip()
        try:
            skuNweight = response.xpath('//meta[@itemprop="sku"]/@content').get().split('_')
        except:
            skuNweight = ['', '']
        while len(skuNweight) < 2:
            skuNweight.append('')
        brand = response.xpath('//div[@role="group"]/a/@title').get()
        try:
            sku = skuNweight[0]
            int(sku)
        except:
        ...
        yield {
            ...
            "Attribute 3": "Type" if terpenes else '',
            "Attribute value 3": terpenes,
            "Attribute 4": "",
            "Attribute value 4": "",
            "Reviews": '',
            "Review link": "",
