...
class TokaCannabisHamilton(BaseSpider):
	name = 'tokacannabishamilton'
	shop_name = 'Toka Cannabis'
	p_id = '642155096'
	start_urls = ['https://tokacannabishamilton.budguide.io/#/main']
	...
	def parse(self, response):
		yield {
			...
			"Description": "",
			"Link": 'https://tokacannabishamilton.budguide.io/#/main',
			"SKU": "",
			"City": '',
			"Province": '',
			...
			"lat": '',
			"ccc": "",
			"Page Url": 'https://tokacannabishamilton.budguide.io/#/main',
			"Active": "",
			"Main image": '',
			"Image 2": "",
			"Image 3": "",
		...
		wss_link = 'wss://s-usc1c-nss-373.firebaseio.com/.ws?v=5&ns=api2-clickspace-tv'
		ws = create_connection(wss_link)
		ws.recv()
		ws.send('{"t":"d","d":{"r":1,"a":"s","b":{"c":{"sdk.js.8-2-6":1}}}}')
		ws.recv()
		ws.send('{"t":"d","d":{"r":2,"a":"q","b":{"p":"/iqmetrix/mergedList/ids/TokaCannabis-dot-Budvue/263081/0fe2c2cb91d4585f6352d0f0af774a75","h":""}}}')
		result = ws.recv()
		ws.recv()
		...
		for id_obj in ids:
			obj_oid = ids[id_obj]
			nb += 1
			msg_send = '{{"t":"d","d":{{"r":{},"a":"q","b":{{"p":"/iqmetrix/mergedList/products/TokaCannabis-dot-Budvue/263081/0fe2c2cb91d4585f6352d0f0af774a75/{}","h":""}}}}}}'.format(
				nb, obj_oid)
			ws.send(msg_send)
			res = ws.recv()
			if str(res) == '2':
