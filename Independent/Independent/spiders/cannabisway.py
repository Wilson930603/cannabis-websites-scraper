...
class CannabisWaySpider(BaseSpider):
	name = 'cannabisway'
	shop_name = 'Cannabis Way'
	p_id = '7387918002'
	start_urls = ['https://cannabisway.budguide.io/#/main']
	...
	def parse(self, response):
		yield {
			...
			"Description": "",
			"Link": 'https://cannabisway.budguide.io/#/main',
			"SKU": "",
			"City": '',
			"Province": '',
			...
			"lat": '',
			"ccc": "",
			"Page Url": 'https://cannabisway.budguide.io/#/main',
			"Active": "",
			"Main image": '',
			"Image 2": "",
			"Image 3": "",
		...
		ws.recv()
		ws.send('{"t":"d","d":{"r":2,"a":"q","b":{"p":"/profitek/mergedList/ids/CannabisWay/ccd3353155246183bf4a35f765c8aa9e","h":""}}}')
		result = ws.recv()
		ws.recv()
		...
		for id_obj in ids:
			obj_oid = ids[id_obj]
			nb += 1
			msg_send = '{{"t":"d","d":{{"r":{},"a":"q","b":{{"p":"/profitek/mergedList/products/CannabisWay/ccd3353155246183bf4a35f765c8aa9e/{}","h":""}}}}}}'.format(
				nb, obj_oid)
			ws.send(msg_send)
			res = ws.recv()
			ws.recv()
			...
			for prod in products:
				...
				if price == old_price:
					old_price = ''
				img = ['', '', '', '', '']
				if 'imageUrls' in prod['partDb']:
					images = prod['partDb']['imageUrls']
					ln = len(images)
				...
				try:
					...
					weight = ''
				desc = ''
				if 'longDescription' in prod['partDb']:
					desc = prod['partDb']['longDescription']
				print(product['id'])
				yield {
					"Page URL": 'https://cannabisway.budguide.io/#/main',
					"Brand": brand,
