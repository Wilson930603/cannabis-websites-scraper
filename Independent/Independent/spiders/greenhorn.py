...
class GreenhornScraper(BaseSpider):
	name = 'greenhorn'
	start_urls = ['https://shop.greenhorncannabis.com/ht/api/objects/kiosk/stores']
	store_id = '9823563'
	...
	def parse(self, response):
		...
		shop = {
			"Producer ID": '',
			"p_id": self.store_id,
			"Producer": f'THE GREENHORN - {store["city"]}',
			"Description": "",
			"Link": "https://shop.greenhorncannabis.com/clickncollect/",
			"SKU": "",
			"City": store["city"],
			"Province": store["state"],
			"Store Name": "THE GREENHORN",
			"Postal Code": store["zip_code"],
			"long": "",
			"lat": "",
			"ccc": "",
			"Page Url": "https://shop.greenhorncannabis.com/clickncollect/",
			"Active": "",
			"Main image": f'https://shop.greenhorncannabis.com/s/{store["logo"]}',
			"Image 2": '',
			"Image 3": '',
			"Image 4": '',
			"Image 5": '',
		...
		yield scrapy.Request('https://greenhorncannabis.com/', callback=self.parse_additional, meta={'shop': shop})
		headers = {'cookie': f'store_id={store["store_id"]}; store_limit={store["cannabis_limit"]}; store_state={store["state"]}; store_city={store["city"]}'}
		yield scrapy.Request('https://shop.greenhorncannabis.com/ht/api/objects/kiosk/products?page_size=2000&page=0', headers=headers, callback=self.parse_products)

	def parse_additional(self, response):
		phone = response.xpath('//div[@data-field="phone"]/ul/li/text()').get()
		postal_code = response.xpath('//address/div[2]/text()').get().split(' ')[-1]
	...
	def parse_products(self, response):
		...
		for product in products:
			...
			if img:
				img = f'https://shop.greenhorncannabis.com{img}'
			...
			if product["on_hand"]:
				...
				if cnt > 0:
					stock = 'In Stock'
			yield {
				"Page URL": f'https://shop.greenhorncannabis.com/clickncollect/shop-cannabis-online/british_columbia/{product["id"]}',
				"Brand": brand,
