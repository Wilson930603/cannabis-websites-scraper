...
class StirlingcannabisScraper(BaseSpider):
	name = 'stirlingcannabis'
	start_urls = ['https://www.stirlingcannabis.ca/']
	shop_name = 'Stirling Cannabis'
	p_id = '881677109'

	def parse(self, response):
		img = response.xpath('//img[@id="logo"]/@src').get()
		desc = ' '.join(response.xpath('//div[@class="et_pb_module et_pb_text et_pb_text_5  et_pb_text_align_left et_pb_bg_layout_dark"]/div/p[1]/text()').getall())
		phone = response.xpath('//div[@class="et_pb_module et_pb_blurb et_pb_blurb_6  et_pb_text_align_left  et_pb_blurb_position_left et_pb_bg_layout_light"]//h4/span/text()').get().replace('-', ' ')
		addr, city_pro, postal_code = response.xpath('//div[@class="et_pb_module et_pb_text et_pb_text_5  et_pb_text_align_left et_pb_bg_layout_dark"]/div/p[2]/text()').get().split(' in ')[1].split(', ')
		...
		yield {
			...
			"Description": desc,
			"Link": 'https://www.stirlingcannabis.ca/',
			"SKU": "",
			"City": city,
			"Province": province,
			...
			"lat": '',
			"ccc": "",
			"Page Url": 'https://www.stirlingcannabis.ca/',
			"Active": "",
			"Main image": img,
			"Image 2": "",
			"Image 3": "",
	...
	def parse_products(self, response):
		...
		for product in products:
			...
			if old_price == price:
				old_price = ''
			yield {
				"Page URL": 'https://www.stirlingcannabis.ca/shop-cannabis/',
				"Brand": brand,
				"Name": product["name"],
				"SKU": product["sku"],
